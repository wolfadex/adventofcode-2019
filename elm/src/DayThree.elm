...
parseActionList : List String -> List Action
parseActionList =
    List.map
        (\act ->
            case Parser.run parseAction act of
                Ok a ->
                    a
...
parseAction =
    Parser.succeed (\dir val -> dir val)
        |= parseDirection
        |= Parser.int


parseDirection : Parser (Int -> Action)
parseDirection =
    (Parser.getChompedString <|
        Parser.succeed ()
            |. Parser.chompIf (\c -> List.member c [ 'U', 'D', 'L', 'R' ])
    )
        |> Parser.andThen
            (\dir ->
                case dir of
                    ...
                    _ ->
                        Parser.problem ("Unknown direction" ++ dir)
            )
